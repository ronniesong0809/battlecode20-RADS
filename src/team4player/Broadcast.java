...
public class Broadcast {
  ...
  public static int broadcastFCCreation() throws GameActionException {
    int packedMessage = 0;
    int[] message = new int[7];
    packedMessage = (packedMessage << 6) + 1;
    packedMessage = (packedMessage << 6) + 1;
    packedMessage = (packedMessage << 6) + FCSecret;
    //System.out.println("SENDING: " + packedMessage);
    message[0] = packedMessage;
  ...
  // Java Best Waifu's sendMessage function. This is built to mess up their blockchain, if possible
  void sendMessage(int type, int code){
    try {
      System.out.println("Sendind message " + type + " " + code);
      int b = 4; // just putting a number out there...
      int MASK = 4534653;
